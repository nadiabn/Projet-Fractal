// g++ -Wall threads-array-time-c.cpp -lopencv_core -lopencv_highgui -lopencv_imgproc -lpthread -std=c++11
...
int num_threads;
int th[8]= {2,8,32,57,128,256,437,512};
...
while(1) {
	for(int n=0; n<8;n++)  
	{
		num_threads=th[n];	
		std::thread tid[num_threads];
		std::cout << "th= " << num_threads << std::endl;
		for(int j=0;j<10;j++) 
		{
	 		if(im<1.) 
			{
			im+=0.02; 
			std::cout << "valeur imaginaire= " << im << std::endl;
			}
			else 
			{
			im-=2;
			...
			{
			re+=0.08; 
			std::cout << "valeur réélle= " << re << std::endl;
			}
			else 
			{
			re-=2;
